...
############### It conducts the item level analyses while modelling for multiple comparisions                                         #################
...
## Now we will have to convert our existing dataframe to a long format so that we can run the big multilevel model as specified by Gelman to appropriately model multiple comparisons

## Let's first drop the columns that we won't using at all
dep_data_long <- dep_data %>%
  select(-c(D, DisabilityScore))  

## Now converting it into a long format where all item scores are under one column
dep_data_long <- dep_data_long %>%
  pivot_longer(
    cols = starts_with("D"),
    names_to = "Item",
    values_to = "ItemScore"
...
## Do a quick sanity check on whether this conversion was successful - Yes, it was!!
dep_data_long %>% count(Item)





########### Specifying the big model now  ##################
#################################################################################################################################

## Specifying the model
m_comparision_mod_formula <- bf(ItemScore ~ 1 + Age + Sex + mi(SCI) + mi(SFluency) + s(Interview_Date) + Town_Distance + Com_Size + mi(HH_Size) + 
                                  (1 + Age + Sex + mi(SCI) + mi(SFluency) + Town_Distance + Com_Size + mi(HH_Size) | Item) +
                                  (1 | Item:Interviewer) + (1 |Item:ComID) + (1 |Item:UniqueFamID) + (1|PID), 
...
## Running the model
m_comparision_mod <- brm(m_comparision_mod_formula, 
            data   = dep_data_long,
            prior = c(prior(normal(0, 1.5), class = "Intercept", resp = ItemScore),
                      prior(normal(0, 0.5), class ="b", resp = ItemScore),
                      prior(exponential(1), class = "sd", resp = ItemScore),
                      prior(student_t(3, 0, 1), class = "sds", coef = s(Interview_Date), resp = ItemScore),
...
saveRDS(m_comparision_mod, "multiple_comparision_model.RDS")

## Load model
m_comparision_mod <- readRDS("multiple_comparision_model.RDS")

## Model summary
summary(m_comparision_mod)
